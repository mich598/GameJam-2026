...
// ── Adjust these two lines to change game length ──
const CUSTOMERS_PER_DAY = 1;   // customers served before day ends
const TOTAL_DAYS        = 2;   // days before the bank scene
...
function submitChange() {
    ...
    if (given === required) {
        ...
        // Track total customers served (used to trigger the minigame alert)
        const totalServed = parseInt(localStorage.getItem('totalCustomersServed') || '0');
        localStorage.setItem('totalCustomersServed', String(totalServed + 1));
        ...
        if (servedToday >= CUSTOMERS_PER_DAY) {
          ...
          if (day >= TOTAL_DAYS) localStorage.setItem('gameComplete', 'true');
