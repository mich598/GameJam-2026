...
let state = {
    score: 0, // Keeping score for potential future use
    changeGivenCents: 0,
    coinsInBox: [],
    submitted: false,
    hintVisible: false,
    ...
    setTimeout(() => {
        ...
        if (state.coinsInBox.length === 0) {
            el('changeBox').innerHTML =
                '<div class="empty-box-hint" id="emptyHint">Click coins on the right to add them →</div>';
        }
...
function updateChangeDisplay() {
    const required = 180; // Hardcoded required change in cents
...
/**
 * Dynamically populate the awning fringe triangles so they
 * fill the screen width without hard-coding a count.
 */
function buildFringe() {
    const fringeEl = document.querySelector('.awning-fringe');
    if (!fringeEl) return;
    const triWidth  = 32; // px per triangle (matches CSS)
    const count     = Math.ceil(window.innerWidth / triWidth) + 2;
    fringeEl.innerHTML = '';
    for (let i = 0; i < count; i++) {
        const tri = document.createElement('div');
        tri.className = 'fringe-tri';
        fringeEl.appendChild(tri);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Rebuild fringe if window is resized
    window.addEventListener('resize', buildFringe);
});
...
function initGame() {
    ...
    el('customerPayment').textContent = '$2.50';
    el('requiredChange').textContent = '$1.80';
    el('changeGivenDisplay').textContent = '$0.00';

    const box = el('changeBox');
    box.innerHTML = '<div class="empty-box-hint" id="emptyHint">Click coins on the right to add them →</div>';
